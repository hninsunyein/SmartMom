...
const inputCls = 'w-full px-4 py-2.5 border-2 border-[#8BA888] rounded-xl focus:outline-none focus:border-[#6D8A6A] focus:ring-2 focus:ring-[#8BA888]/20 text-sm transition-colors';
const labelCls = 'block text-[#2C3E50] font-semibold mb-1.5 text-sm';
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
        ...
        <div className="p-6">
          <p className="text-[#2C3E50] text-sm mb-6">{message}</p>
          <div className="flex gap-3">
            <button onClick={onCancel}
              className="flex-1 py-2.5 border-2 border-[#8BA888] text-[#8BA888] font-semibold rounded-xl hover:bg-[#8BA888]/5 active:scale-95 transition-all text-sm">
              Cancel
            </button>
...
function AdvisorCard({ advisor, onApprove, onReject, onDelete }) {
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-2xl shadow-sm overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-[#8BA888] to-[#6D8A6A] px-5 py-4 flex items-start justify-between gap-3">
        <div>
      ...
      <div className="p-5">
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-[#F8FAFC] rounded-xl px-3 py-2.5">
            <p className="text-[#94A3B8] text-xs mb-0.5">Specialization</p>
            <p className="font-semibold text-[#2C3E50] text-sm">{advisor.specialty || '—'}</p>
          </div>
          <div className="bg-[#F8FAFC] rounded-xl px-3 py-2.5">
            <p className="text-[#94A3B8] text-xs mb-0.5">Experience</p>
            <p className="font-semibold text-[#2C3E50] text-sm">{advisor.experienceYears ? `${advisor.experienceYears} yrs` : '—'}</p>
          </div>
          <div className="bg-[#F8FAFC] rounded-xl px-3 py-2.5">
            <p className="text-[#94A3B8] text-xs mb-0.5">License No.</p>
            <p className="font-semibold text-[#2C3E50] text-sm">{advisor.licenseNumber || '—'}</p>
          </div>
          <div className="bg-[#F8FAFC] rounded-xl px-3 py-2.5">
            <p className="text-[#94A3B8] text-xs mb-0.5">Phone</p>
            <p className="font-semibold text-[#2C3E50] text-sm">{advisor.phone || '—'}</p>
          </div>
        </div>
        ...
        {advisor.availability && advisor.availability.length > 0 && (
          <div className="mb-4">
            <p className="text-xs font-semibold text-[#2C3E50] mb-2 uppercase tracking-wide">Availability</p>
            <div className="flex flex-wrap gap-1.5">
              {advisor.availability.map((a, i) => (
                <span key={a.id ?? i}
                  className="text-xs px-2.5 py-1 rounded-lg bg-[#F0F9F5] text-[#8BA888] border border-[#8BA888]/30 capitalize font-medium">
...
function TipRow({ tip, onEdit, onDelete }) {
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 flex items-start justify-between gap-4 shadow-sm">
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap gap-1.5 mb-1.5">
          <span className="text-[0.7rem] px-2 py-0.5 bg-[#F0F9F5] text-[#8BA888] rounded-lg font-bold">{tip.category}</span>
          {tip.ageGroup && (
            <span className="text-[0.7rem] px-2 py-0.5 bg-[#F1F5F9] text-[#64748B] rounded-lg">{tip.ageGroup}</span>
          )}
        </div>
        <p className="font-semibold text-[#2C3E50] text-sm">{tip.title}</p>
        <p className="text-[#94A3B8] text-xs mt-1 line-clamp-2">{tip.content}</p>
      </div>
      <div className="flex gap-2 shrink-0">
        <button onClick={() => onEdit(tip)}
          className="bg-gradient-to-r from-[#8BA888] to-[#6D8A6A] text-white text-xs font-semibold px-3 py-1.5 rounded-lg hover:opacity-90 active:scale-95 transition-all">
          Edit
        </button>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-[#8BA888] to-[#6D8A6A] text-white font-bold px-6 py-4">
          Edit Tip
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
              className="flex-1 py-3 border-2 border-[#8BA888] text-[#8BA888] font-semibold rounded-xl hover:bg-[#8BA888]/5 active:scale-95 transition-all text-sm">
              Cancel
            </button>
            <button type="submit" disabled={saving}
              className="flex-1 bg-gradient-to-r from-[#8BA888] to-[#6D8A6A] text-white font-bold py-3 rounded-xl hover:opacity-90 active:scale-95 transition-all disabled:opacity-55 text-sm">
...
export default function AdminTab({ initialSection = 'advisors' }) {
  ...
  return (
    <div>
      {/* Header */}
      <div className="bg-gradient-to-r from-[#8BA888] to-[#6D8A6A] text-white font-bold px-5 py-3 rounded-xl mb-5 shadow text-lg">
        Admin Panel
      </div>
        ...
        {[
          ...
          <button key={id} onClick={() => switchSection(id)}
            className={activeSection === id
              ? 'bg-gradient-to-r from-[#8BA888] to-[#6D8A6A] text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow'
              : 'bg-white border-2 border-[#8BA888] text-[#8BA888] font-semibold px-5 py-2.5 rounded-xl text-sm hover:bg-[#8BA888]/5 active:scale-95 transition-all'}>
            {label}
          </button>
        ))}
      </div>
      ...
      {activeSection === 'advisors' && (
        <div>
              ...
              {[
                ...
                <button key={key} onClick={() => setAdvisorFilter(key)}
                  className={`px-4 py-1.5 rounded-xl text-sm font-semibold transition-all border-2 ${
                    advisorFilter === key
                      ? 'bg-gradient-to-r from-[#8BA888] to-[#6D8A6A] text-white border-transparent shadow'
                      : 'bg-white border-[#E2E8F0] text-[#64748B] hover:border-[#8BA888] hover:text-[#8BA888]'
                  }`}>
                  {label}
                  <span className={`ml-1.5 text-xs font-bold px-1.5 py-0.5 rounded-full ${
                    advisorFilter === key ? 'bg-white/20 text-white' : 'bg-[#F1F5F9] text-[#64748B]'
                  }`}>{counts[key]}</span>
                </button>
              ))}
            </div>
          ...
          {advisorLoading && <p className="text-center text-[#94A3B8] py-10">Loading advisors...</p>}
          ...
          {!advisorLoading && filteredAdvisors.length === 0 && (
            <div className="bg-white border border-[#E2E8F0] rounded-2xl p-12 text-center shadow-sm">
              <div className="w-12 h-12 rounded-full bg-[#F0F9F5] flex items-center justify-center mx-auto mb-3">
              ...
              <p className="text-[#64748B] font-semibold">No {advisorFilter === 'all' ? '' : advisorFilter} advisors found</p>
            </div>
          )}
      ...
      {(activeSection === 'safety' || activeSection === 'health') && (
        <div>
          ...
          <div className="flex gap-2 mb-5">
            <button onClick={() => setTipView('list')}
              className={tipView === 'list'
                ? 'bg-gradient-to-r from-[#8BA888] to-[#6D8A6A] text-white font-bold px-4 py-2 rounded-xl text-sm shadow'
                : 'bg-white border-2 border-[#8BA888] text-[#8BA888] font-semibold px-4 py-2 rounded-xl text-sm hover:bg-[#8BA888]/5 transition-all'}>
          ...
          {tipView === 'list' && (
            <div>
              {tipsLoading && <p className="text-center text-[#94A3B8] py-10">Loading...</p>}
              {!tipsLoading && tipsList.length === 0 && (
                <div className="bg-white border border-[#E2E8F0] rounded-2xl p-12 text-center shadow-sm">
                  <p className="text-[#64748B] font-semibold">No {activeSection} tips yet</p>
                  <p className="text-[#94A3B8] text-sm mt-1">Click "+ Add New Tip" to publish your first tip.</p>
                </div>
              )}
          ...
          {/* Add form */}
          {tipView === 'add' && (
            <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 shadow-sm max-w-[600px]">
              <p className="font-bold text-[#2C3E50] text-base mb-5">
